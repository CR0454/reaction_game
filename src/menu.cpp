...
{
    std::regex pattern("^(000[1-9]|001[0-9]|0020)$"); // all numbers between 0010 and 0020 (only in this format)
    ...
    while(true){
        ...
        if(std::regex_match(sequence, pattern)){ //the user has to enter the number by following the pattern
            ...
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            std::cout<<"Invalid expression, please enter a number between 0001 and 0020 (in the same format)\n";
        }
    }
}
